...
export const FalloutLeaderboardUsers = (props: Props) => {
  const { className } = props;

  const dataResult: UseQueryResult<string | undefined> = useQuery({
    queryKey: ["GetNewLeaderboardUsers"],
    queryFn: async () => {
      const resJson = await (await fetch("/api/falloutLeaderboardUsers"))
        .json()
        .catch((err) => {});
  ...
  return (
    ...
    >
      <div className="w-[300px] text-text2">Fallout Leaderboard Users</div>
